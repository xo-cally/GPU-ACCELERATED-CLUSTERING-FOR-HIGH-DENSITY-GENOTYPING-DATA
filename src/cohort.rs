...
pub fn run_from_pairs(
    ...
    // stats per sample
    #[derive(Default)]
    struct PS { total:u64, pass_raw:u64, nocall_raw:u64, nocall_p:u64, calls_tau:u64, calls_p090:u64 }
    let mut stats: Vec<PS> = (0..s).map(|_| PS::default()).collect();

    // Decide tau
    let mut tau_opt: Option<f64> = match cfg.p_mode { PMMode::Fixed(v) => Some(v), PMMode::Auto { .. } => None };
    let alpha_default = match cfg.p_mode { PMMode::Auto { alpha } => alpha, _ => 0.005 };

    // label for plots (no leaks)
    let feature_label: String = match cfg.feature_space {
        FeatureSpace::XYLog1p => "ln(R) vs ln(G)".to_string(),
        FeatureSpace::ThetaR { w_theta, w_r } => format!("θ·wθ={:.2} vs r·w_r={:.2}", w_theta, w_r),
    };
    ...
    for chunk_start in (0..m).step_by(block) {
        ...
        #[derive(Clone)]
        struct Row { id:u32, raw:Vec<u32>, labs:Vec<usize>, post:Vec<f64>, means:Vec<(f64,f64)> }
            ...
            .map(|j| {
                ...
                let model = {
                    ...
                    if cfg.k_max <= 1 { g1 }
                        ...
                        let gk = fit_diag_restarts(&pts, cfg.k_max.min(3), cfg.max_iters, cfg.tol, cfg.seed, cfg.restarts);
                        // pick best with BIC vs K=1
                        let b1 = bic(&g1, pts.len()); let bk = bic(&gk, pts.len());
                        if b1 <= bk { g1 } else { gk }
                    }
                };
                ...
                for si in 0..s {
                    ...
                    if m2.sqrt() > cfg.max_maha { post[si] = 0.0; }
                }
        ...
        for r in rows {
            ...
            let names: Vec<&'static str> = match r.means.len() {
                ...
                _ => {
                    ...
                    n[idx[0].0] = "BB"; n[idx[1].0] = "AB"; n[idx[2].0] = "AA"; n
                }
            };
            ...
            for si in 0..s {
                ...
                st.total += 1;
                if r.raw[si] < r_min {
                    st.nocall_raw += 1; out.push("NoCall".into()); continue;
                }
                st.pass_raw += 1;
                ...
                if r.post[si] < tau {
                    st.nocall_p += 1; out.push("NoCall".into());
                } else {
                    st.calls_tau += 1;
                    let jlab = r.labs[si];
            ...
            if cfg.plot_first_snps > 0 && plots_emitted < cfg.plot_first_snps {
                ...
                for si in 0..s {
                    ...
                    let lnr = ln_x[si][p]; let lng = ln_y[si][p];
                    pts.push(to_features_from_logs(&cfg.feature_space, lnr, lng));
                }
                let plot_path_base = plots_dir.join(format!("snp_{:08}", r.id));
                let _ = write_cluster_scatter_png(
                    &plot_path_base, r.id, &feature_label, &pts, &r.labs, &r.means, &names,
                );
                plots_emitted += 1;
            }
        }
    ...
    qcw.write_record(&[
        "Sample","total_snps","pass_raw","nocall_raw","nocall_p_at_tau","calls_at_tau","calls_at_p090",
        "call_rate_tau","call_rate_p090","r_min_used","tau_used",
    ...
    for (i, st) in stats.iter().enumerate() {
        ...
        qcw.write_record(&[
            name.to_string(), st.total.to_string(), st.pass_raw.to_string(),
            st.nocall_raw.to_string(), st.nocall_p.to_string(),
            st.calls_tau.to_string(), st.calls_p090.to_string(),
            format!("{:.6}", rate_tau), format!("{:.6}", rate_p),
            r_min.to_string(), format!("{:.4}", tau_report),
        ]).map_err(|e| e.to_string())?;
    }
